// https://apptesters.org/wp-content/uploads/AutoRC.js
const $ = new Env("Revenuecat");
...
(async () => {
  ...
  // $.log(`resp.body: ${data.body}`);

  const ent = JSON.parse(data.body);
  ...
  for (const [entitlementId, productInfo] of Object.entries(productEntitlementMapping)) {
    ...
    $.log(`productIdentifier: ${productIdentifier}`)
